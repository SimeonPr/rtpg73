...
impl Request {
    ...
    /// Merge self with r2 by adding id to acknowledgements.
...
impl WorldView {
    ...
    pub fn handle_humbly(&self, foreign_world_view: WorldView) -> WorldView {
        ...
        if let Some(foreign_elev) = new_wv.elevators.get_mut(&id) {
            if let Some(local_elev) = self.elevators.get(&id) {
                for floor in 0..config::FLOOR_COUNT {
                    foreign_elev.cab_requests[floor].merge(&local_elev.cab_requests[floor], id);
                }
            }
    ...
    pub fn handle_foreign_world_view(&self, foreign_world_view: WorldView) -> (WorldView, bool) {
        ...
        // Update the state of the foreign elevator.
        ...
        // Update barrier states (filtering out dead acks).
        let (new_wv, barrier_updated) = new_wv.update_states_at_barrier();
    ...
    pub fn update_states_at_barrier(&self) -> (WorldView, bool) {
        ...
        // For hall requests, build a filtered ack set.
        for floor in 0..config::FLOOR_COUNT {
            for dir in 0..2 {
                if let RequestState::Unconfirmed = new_wv.hall_requests[floor][dir].state {
                    let working_acks: HashSet<u8> = new_wv.hall_requests[floor][dir]
                        .acks
                        .iter()
                        .copied()
    ...
    // Return the hall requests (same type as stored)
...
pub fn run(
    ...
    loop {
        ...
        if updated && humble_counter == 0 {
            inform_everybody(&mut world_view, &sender_tx, &controller_tx, &lights_tx);
        }
    }
}
...
fn inform_everybody(
    ...
    sender_tx
        .send(messages::Manager::HeartBeat(world_view.clone()))
        .expect("send to sender failed");
    ...
    controller_tx
        .send(messages::Controller::Requests(controller_reqs))
        .expect("send to controller failed");

    let lights_reqs = world_view.get_confirmed_requests();
    lights_tx
        .send(messages::Controller::Requests(lights_reqs))
        .expect("send to lights failed");
}
